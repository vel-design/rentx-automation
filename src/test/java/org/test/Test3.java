...
public class Test3 {
    ...
    public void clickNextSmart() {
        ...
        if (next == null) {
            ...
            nap(120); next = findNextLikeButtonOrNull();
        }
        if (next == null) {
    ...
    public void fillCustomerDetails(String first, String last, String email, String phone, boolean agreeTerms) {
        ...
        if (agreeTerms) {
            WebElement label = findVisible(By.cssSelector("label.custom-checkbox"));
            WebElement input = label.findElement(By.cssSelector("input[type='checkbox'][name='terms']"));
            if (!input.isSelected()) safeClick(label);
            if (!input.isSelected()) safeClick(input); // fallback
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Test3 s = new Test3(driver);

            // Step 1 -> 2
            s.runFlowWithAddressIndices(
                    "l",              // category search
                    "19-09-2025",     // date (dd-MM-yyyy)
                    "01:00 PM",       // time
            ...
            // Step 3
            s.waitForStep3();
            s.fillCustomerDetails("Test Vel", "Test Mobo", "test@mobo.com", "08754140962", true);
            s.clickSubmit();
