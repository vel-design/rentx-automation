...
public class Script {
    ...
    public void chooseCategory(String searchTerm) {
        ...
        // suggestions list id from aria-controls (e.g., autoComplete_list_1/_2/…)
    ...
    public void pickTimeByText(String timeTextDesired) {
        ...
        // collect all "Select" buttons (each row should have one)
        ...
        // iterate rows and compare normalized text
        ...
        // Fallback: click the first available time
    ...
    private List<WebElement> findAddressOptionsForInput(WebElement input) {
        // 1) If the input exposes aria-controls, use that container first
        String listId = input.getAttribute("aria-controls");
        if (listId != null && !listId.isBlank()) {
            By optInList = By.cssSelector(
                "#" + listId + " [role='option'], " +     // ARIA options
                "#" + listId + " li, " +                  // list items
                "#" + listId + " .list-group-item, " +    // Bootstrap-ish
                "#" + listId + " .autocomplete-item"      // generic
            );
            List<WebElement> inside = driver.findElements(optInList);
            if (!inside.isEmpty()) return inside;
        }

        // 2) Common autocomplete containers near the input
        // (closest following dropdown/autocomplete)
        try {
    ...
    public String selectAddressByIndex(String inputId, String query, int n) {
        ...
        // Poll until options appear (handles async/autocomplete latency)
        ...
        // choose n-th option (clamp to size)
        int idx = Math.min(n - 1, options.size() - 1);
        WebElement option = options.get(idx);

        // if the row has an inner clickable anchor/button, prefer it; otherwise click the row
        ...
        // confirm the input got populated
        wait.until(d -> {
    ...
    /** Full flow with address-by-index picking on Step 2. */
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Script selector = new Script(driver);
            

            // Example: choose category "l", date 19-09-2025, time 01:00 PM,
            // then pick the 1st pickup suggestion for "st" and the 2nd drop suggestion for "12"
            selector.runFlowWithAddressIndices(
                "l",
                "19-09-2025",
                "01:00 PM",
                "street", 2,       // pickup: first suggestion
                "12345", 2        // drop: second suggestion (clamps to last if only one exists)
                      );  
